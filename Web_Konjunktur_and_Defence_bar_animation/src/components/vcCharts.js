...
function renderGermanyCharts(countryData) {
  ...
  if (getElement('deutschland-funding')) {
    ...
    const options = Highcharts.merge(columnChartOptions(), {
      ...
      series: [{ name: 'Finanzierung', data: fundingData, color: chartColors.accent }],
      tooltip: { pointFormat: '<b>{point.y:.0f} Mio. USD</b>' },
    });
    const animatedOptions = prepareViewportBarChart('deutschland-funding', options, [fundingData]);
    const chart = Highcharts.chart('deutschland-funding', animatedOptions);
    registerViewportBarChart('deutschland-funding', chart);
  }

  if (getElement('deutschland-share')) {
    const shareData = [share2019, share2024];
    const options = Highcharts.merge(columnChartOptions(), {
      ...
      series: [{ name: 'Anteil', data: shareData, color: chartColors.accentWarm }],
      tooltip: { pointFormat: '<b>{point.y:.1f}%</b>' },
    });
    const animatedOptions = prepareViewportBarChart('deutschland-share', options, [shareData]);
    const chart = Highcharts.chart('deutschland-share', animatedOptions);
    registerViewportBarChart('deutschland-share', chart);
  }
}
...
function renderVerticalCharts(verticalData) {
  ...
  if (getElement('funding-chart')) {
    ...
    const options = Highcharts.merge(columnChartOptions(), {
      ...
      series: [{ name: '2025 Funding', data: fundingData, color: chartColors.accent }],
      tooltip: {
        formatter() {
    ...
    const animatedOptions = prepareViewportBarChart('funding-chart', options, [fundingData]);
    const chart = Highcharts.chart('funding-chart', animatedOptions);
    registerViewportBarChart('funding-chart', chart);
  }

  if (getElement('growth-chart')) {
    const growthData = verticals.map((entry) => entry.growth);
    const growthMin = Math.min(...growthData, 0);
    const growthMax = Math.max(...growthData, 0);
    const options = Highcharts.merge(columnChartOptions(), {
      ...
      yAxis: {
        title: { text: 'Wachstum (%)' },
        tickInterval: 20,
        min: growthMin < 0 ? growthMin * 1.18 : 0,
        max: growthMax > 0 ? growthMax * 1.12 : 0,
    ...
    const animatedOptions = prepareViewportBarChart('growth-chart', options, [growthData]);
    const chart = Highcharts.chart('growth-chart', animatedOptions);
    registerViewportBarChart('growth-chart', chart);
  }
}
